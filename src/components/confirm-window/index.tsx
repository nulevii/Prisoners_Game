...
const ConfirmWindow: React.FC<{ onActionYes: () => void, onActionNo: () => void, action: string }> = function ({ onActionYes, onActionNo, action }) {
  ...
  return (
    <div className='modal-window'>
      ...
      <button className='confirm__button yes-btn' onClick={onActionYes} ref={(el) => { textShadowRefs.current![0] = el! }}></button>
      <button className='confirm__button no-btn' onClick={onActionNo} ref={(el) => { textShadowRefs.current![1] = el! }}></button>
      </div>
    </div>
  )
}
