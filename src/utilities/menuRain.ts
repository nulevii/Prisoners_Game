...
const createRain = (
  ...
  rainCondition: MutableRefObject<number>
): any => {
  ...
  function drawRainTrough (i: number): void {
    ctx1?.beginPath()
    const grd: any = ctx1?.createLinearGradient(
      0,
      RainTrough[i].y,
      0,
...
function useRainMenu (
  ...
  const rainCondition: MutableRefObject<number> = useRef(0)
  useEffect(() => {
    const resizeRain = createRain(rainCanvasRefs, rainCondition)
    return () => {
      cancelAnimationFrame(rainCondition.current)
      removeEventListener('resize', resizeRain)
    }
  }, [])
  return rainCanvasRefs
}
// return createRainFrame()
